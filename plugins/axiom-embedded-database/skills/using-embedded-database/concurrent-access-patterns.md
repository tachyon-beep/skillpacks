...
**Checkpoint.** Periodically, the WAL is folded back into the main database file in a process called checkpointing. By default, SQLite triggers an automatic checkpoint when the WAL reaches 1000 pages (`wal_autocheckpoint`). Checkpointing is driven by the writer (it happens as part of commit-path logic), but can also be triggered manually. During a checkpoint, readers are not blocked — they continue reading from either the main file or the WAL as before. Writers are blocked briefly while the checkpoint rewrites WAL pages into the main file for the portion of the WAL that no reader currently holds a snapshot over.
