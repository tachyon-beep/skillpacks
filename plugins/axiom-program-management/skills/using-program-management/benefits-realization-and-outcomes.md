...
## Adoption is the bridge from output to outcome — and it is work

The output→outcome arrow is the one this sheet leans on hardest, and it has a name: **adoption**. An output produces an outcome only when the people it is for actually change what they do — and that change is not caused by the feature shipping. It is caused by the *adoption work*: readiness (are the users prepared for the change?), enablement (training, documentation, support through the transition), communication (do they know it exists, and why it helps them?), and reinforcement (does the new behaviour stick once attention moves on, or does the old one reassert itself?). A program that funds the build to 100% and adoption to 0% has bought the output and gambled the outcome — and "nobody adopted it" is the single most common way the output–outcome gap goes total.

This is its own discipline — organizational change management (ADKAR and Kotter are the reference literature) — and at scale it is a staffed workstream with its own plan, not a launch email. **This pack does not define that discipline; it insists you account for it.** Two parts of it are within reach of this pack: the engagement and communication mechanics live in [`stakeholder-and-communication.md`](stakeholder-and-communication.md), and the *desirability* of the change — whether users want the new behaviour at all — is product-discovery work in `/lyra-ux-designer`. The rest (training, process redesign, readiness, reinforcement) is real delivery scope: put it on the roadmap and the benefits map as the work that makes the output→outcome arrow fire, give it an owner, and forecast it like any other workstream rather than assuming it is free.
...
7. **Adoption assumed free.** The output ships and the outcome is expected to follow automatically — no readiness, enablement, communication, or reinforcement budgeted — so the behaviour change the benefit depends on never happens and the gap goes total. *Fix: treat adoption (change management) as funded delivery scope with an owner and a place on the roadmap and benefits map; route the engagement mechanics to `stakeholder-and-communication.md` and the desirability question to `/lyra-ux-designer`, and plan training, readiness, and reinforcement as the work that makes output→outcome fire.*
...
- [stakeholder-and-communication.md](stakeholder-and-communication.md) — the engagement and communication half of adoption: the output→outcome arrow fires only if the people it is for change what they do, and that change is driven by the stakeholder and communication mechanics there, not by the feature shipping.
