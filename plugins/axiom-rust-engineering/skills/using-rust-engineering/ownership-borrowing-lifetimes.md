...
NLL was enabled for the 2018 edition in Rust 1.31 (Dec 2018) and extended to all editions in Rust 1.36 (Jul 2019). With **Non-Lexical Lifetimes**, a borrow ends at its **last use**, not at the end of the enclosing lexical scope. This eliminates many false-positive borrow checker errors from earlier Rust.
...
Without NLL (pre-1.31), the borrow of `first` would have extended to the end of the block, making `data.push(4)` a compile error.
...
### Decision: `T` vs `&T` vs `&mut T` vs `Arc<T>`

The canonical "what should my parameter/field type be?" decision. Walk the tree top to bottom:

```
Do I need to own the value (store it long-term or consume it)?
├─ Yes, and I need to mutate it                                  → T (owned)
├─ Yes, but I only read it                                       → T (owned)
└─ No (I'm only borrowing for the duration of a call/scope)
   │
   ├─ Is the data shared across threads that run concurrently?
   │   ├─ Yes — immutable sharing                                → Arc<T>
   │   ├─ Yes — shared mutation needed                           → Arc<Mutex<T>> / Arc<RwLock<T>>
   │   └─ No (single-threaded or strict handoff)
   │       │
   │       ├─ Need to mutate?
   │       │   ├─ Yes, exclusive access                          → &mut T
   │       │   └─ No, read only                                  → &T
   │       └─ Shared in a single thread with interior mutability → Rc<RefCell<T>>
```

**Default order of preference**, from cheapest to most expensive: `&T`, `&mut T`, `T`, `Arc<T>`, `Arc<Mutex<T>>`. Reach for `Arc`/`Mutex` only when the simpler options actually cannot express the design — not to silence the borrow checker.
...
- Use `Pin<Box<T>>` with `unsafe` or the `ouroboros` crate. (Note: `pin-project` safely *projects through* `Pin` but does not create self-referential structs — it is a separate tool for a separate job.)
...
// Async state machines that borrow across `await` points are !Unpin
// (trivial async blocks with no cross-await borrows can still be Unpin)
```
...
3. **Explicit self-referential structs** via `ouroboros` (safe proc-macro that builds the self-referential structure for you). Note: `pin-project` is for safely projecting through `Pin`, not for creating self-referential structures.
...
**The fix**: Prefer index-based access over internal references. When genuine self-reference is needed, use `ouroboros` (safe proc-macro that builds a self-referential type for you) or hand-rolled `Pin<Box<T>>` with `PhantomPinned` and explicit `unsafe` invariants. `pin-project` is not the tool here — it safely projects *through* `Pin`, which is a different problem. Document the invariant thoroughly.
...
- [traits-generics-and-dispatch.md](traits-generics-and-dispatch.md) — Trait bounds that involve lifetimes: `T: 'a`, higher-ranked trait bounds (`for<'a>`), dyn compatibility (formerly object safety) and `dyn Trait` lifetime rules.
