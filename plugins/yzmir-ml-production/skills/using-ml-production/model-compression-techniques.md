...
### Structured / Semi-Structured Pruning for LLMs

For decoder-only LLMs at 7B-70B scale, the cost of full retraining-after-pruning is usually prohibitive. Two one-shot methods anchor current practice:

- **SparseGPT** (Frantar & Alistarh, 2023) — second-order, layer-wise reconstruction; iteratively applies an Optimal Brain Surgeon update to prune and update weights column-wise. Reaches 50%+ unstructured / 2:4 / 4:8 sparsity with minimal task-quality loss on 100B+ parameter models. Paper: [Frantar & Alistarh 2023](https://arxiv.org/abs/2301.00774). Repo: [IST-DASLab/sparsegpt](https://github.com/IST-DASLab/sparsegpt).
- **Wanda** (Sun et al., 2023) — prunes weights by `|w| * ||x||` (weight magnitude scaled by per-input-channel activation norm), per-output. No weight update, no inverse Hessian, no retraining. Roughly 300x faster than SparseGPT and competitive on zero-shot benchmarks. Paper: [Sun et al. 2023, ICLR 2024](https://arxiv.org/abs/2306.11695). Repo: [locuslab/wanda](https://github.com/locuslab/wanda).

**Practical guidance:**
- For NVIDIA Tensor Cores, target **2:4 semi-structured sparsity** — gives ~1.6x linear-layer speedup (per Sun et al. 2023) and is hardware-supported on Ampere and later.
- Always evaluate task-quality on representative downstream evals, not just perplexity. A 5% perplexity bump on WikiText can hide a 20% drop on instruction-following or code generation.
- Cross-ref `quantization-for-inference.md` — pruning + 4/8-bit quantization compose well, often near-multiplicatively for memory.
...
### Distillation Pointers for LLMs

For LLMs, distillation usually means **supervised fine-tuning of a smaller base model on outputs (or output distributions) generated by a larger teacher**. Cross-ref `yzmir-llm-specialist/llm-finetuning-strategies.md` for the SFT/DPO machinery — the rest of the LLM-distillation pipeline is fine-tuning. Anchor pre-distilled checkpoints to start from:

- **MiniLM** (Wang et al. 2020) — task-agnostic deep self-attention distillation of pretrained transformers; the 12-layer 384-hidden model achieves ~2.7x speedup with comparable NLU quality to BERT-base. Paper: [Wang et al. 2020](https://arxiv.org/abs/2002.10957). Repo: [microsoft/unilm/minilm](https://github.com/microsoft/unilm/tree/master/minilm). MiniLM-L6 family is widely used as a sentence-embedding backbone.
- **TinyLlama** (Zhang et al. 2024) — open 1.1B Llama-architecture model pretrained on ~3T tokens; strong starting point for resource-constrained LLM distillation targets. Paper: [Zhang et al. 2024](https://arxiv.org/abs/2401.02385). Repo: [jzhang38/TinyLlama](https://github.com/jzhang38/TinyLlama).
- **DeepSeek-R1-Distill family** — dense models (Qwen-2.5 1.5B / 7B / 14B / 32B and Llama-3.1 8B / Llama-3.3 70B bases) fine-tuned by DeepSeek on ~800k reasoning traces sampled from DeepSeek-R1, distilling reasoning behavior via SFT only (no RL). Repo: [deepseek-ai/DeepSeek-R1](https://github.com/deepseek-ai/DeepSeek-R1); checkpoints under [`deepseek-ai/DeepSeek-R1-Distill-*`](https://huggingface.co/deepseek-ai) on Hugging Face. Useful as a reference recipe (SFT on synthesized teacher traces) more than as off-the-shelf weights.

**Practical guidance:**
- **Capacity floor matters more than method.** Below ~1B parameters, reasoning distillation degrades sharply; below ~300M, expect retrieval-augmented prompts to help more than distillation.
- **License terms vary** across teacher and student model families — verify redistribution and downstream-fine-tuning rights before publishing distilled weights.
- **Evaluation drift:** task-quality benchmarks for the teacher may not transfer to the student family. Always run downstream evals matched to your deployment task.
...
def full_compression_pipeline(
    ...
    # Quantization-aware training (use torch.ao.quantization, not the legacy torch.quantization)
    pruned_model.qconfig = torch.ao.quantization.get_default_qat_qconfig('fbgemm')
    model_prepared = torch.ao.quantization.prepare_qat(pruned_model)
...
**Key papers:**
- DistilBERT (Sanh et al., 2019): Knowledge distillation for transformers — https://arxiv.org/abs/1910.01108
- MiniLM (Wang et al., 2020): Task-agnostic self-attention distillation — https://arxiv.org/abs/2002.10957
- The Lottery Ticket Hypothesis (Frankle & Carbin, 2019): Iterative magnitude pruning — https://arxiv.org/abs/1803.03635
- Pruning Filters for Efficient ConvNets (Li et al., 2017): Structured channel pruning — https://arxiv.org/abs/1608.08710
- Deep Compression (Han et al., 2016): Pruning + quantization + Huffman coding — https://arxiv.org/abs/1510.00149
- SparseGPT (Frantar & Alistarh, 2023): One-shot LLM pruning — https://arxiv.org/abs/2301.00774
- Wanda (Sun et al., 2023): Pruning by weights and activations — https://arxiv.org/abs/2306.11695
- TinyLlama (Zhang et al., 2024): 1.1B open Llama-architecture model — https://arxiv.org/abs/2401.02385
...
- Use with `yzmir-llm-specialist/llm-finetuning-strategies.md` for distillation-as-fine-tuning (SFT on teacher outputs, DPO from teacher preferences)
